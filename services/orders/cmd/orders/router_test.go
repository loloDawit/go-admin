...
func TestPanickingHandlerStillProducesALogLineWithStatus500(t *testing.T) {
	...
	handler := platformcheck.NewHandler(platformcheck.NewService(&alwaysFailRepo{}), "orders", httperr.Write)
...
func TestStartupContractHealthzSkipsTheDatabaseAndReadyzReportsFailureSafely(t *testing.T) {
	...
	repo := &alwaysFailRepo{}
	handler := platformcheck.NewHandler(platformcheck.NewService(repo), "orders", httperr.Write)
