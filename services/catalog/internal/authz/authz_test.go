...
func TestRequireRefusesAPrincipalWithoutThePermission(t *testing.T) {
	...
	chainThroughPrincipal(onErr, permission.ViewProducts, next).ServeHTTP(rec, req)
...
// A nil permission set must refuse the same as any other missing permission,
// not panic or fall through slices.Contains on a nil slice.
func TestRequireRefusesAPrincipalWithANilPermissionSet(t *testing.T) {
	req := signedRequest(t, principal.Principal{StaffID: "1", Permissions: nil})
	...
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Fatal("next must not run with a nil permission set")
	})
	rec := httptest.NewRecorder()
	chainThroughPrincipal(onErr, permission.ViewProducts, next).ServeHTTP(rec, req)
...
func TestRequireRefusesAnAbsentPrincipal(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	authz.Require(permission.ViewProducts, onErr)(next).ServeHTTP(rec, req)
...
func TestRequireAcceptsThePermission(t *testing.T) {
	req := signedRequest(t, principal.Principal{StaffID: "1", Permissions: []string{string(permission.ViewProducts)}})
